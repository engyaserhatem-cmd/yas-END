...
interface HomeSafeDisplayProps {
  safeAccounts: Account[];
  selectedAccountId: string;
  onSelect: (id: string) => void;
}

const HomeSafeDisplay: React.FC<HomeSafeDisplayProps> = ({ safeAccounts, selectedAccountId, onSelect }) => {
  const sortedAccounts = [...safeAccounts].sort((a, b) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-3 flex flex-col h-full">
      <h3 className="font-bold text-base mb-2 text-gray-900 text-center">الصندوق المنزلي (نقد)</h3>
...
export default HomeSafeDisplay;